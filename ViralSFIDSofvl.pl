...
my $InputFile;
my $fresh = 0; #Generate a fresh copy of the data?
...
if(-e './.NCBIvlSpeciesComb.dat' && ! $fresh){
	$SpeciesCombs = EasyUnDump('./.NCBIvlSpeciesComb.dat');
}else{
	splitIntoNCBISpeciesCombIDs($NCBIgenome, $SpeciesCombs, $fresh);
...
foreach my $ViralSpecie (keys(%$SpeciesCombs)){
	...
	my $Superfamilies = {};
	#A simple hash count of SF=>no. times observed
	my @Combs = @{$SpeciesCombs->{$ViralSpecie}};

		foreach my $combid (@Combs){
					
					no warnings 'uninitialized';	
	
					my $query = $dbh->prepare_cached("SELECT sf FROM ass JOIN comb ON ass.protein=comb.protein WHERE comb.comb_id = ?") or die;
	    			$query->execute($combid);
