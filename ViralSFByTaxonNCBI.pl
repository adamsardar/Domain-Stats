...
my $OutputFilename = 'NCBISuperFamiliesByViralDistinction.dat';
my $InputFile;
...
foreach my $ViralSpecie (keys(%$SpeciesCombs)){
	...
	my @Combs = @{$SpeciesCombs->{$ViralSpecie}};
		...
		foreach my $comb (@Combs){
			...
			my @Domains = split(',',$comb);
		
			foreach my $domain (@Domains){
				
				unless($domain =~ /_gap_/ && $CompleteCombsOnly){
				
					my $query = $dbh->prepare_cached("SELECT classification FROM des WHERE id = ?") or die;
...
my $No_Of_Species_Total = keys(%$TaxIDs_SFs);
	...
	while(my($Distinction,$left_id,$right_id) = $sth->fetchrow_array()){
		...
		my $query = $dbh->prepare_cached("SELECT taxon_id, name FROM ncbi_taxonomy WHERE left_id > ? AND right_id < ? AND rank = 'species';");
		$query->execute($left_id,$right_id);
		
			print OUTPUT "\n".'>'.$Distinction."\n\n";
		
			while(my ($CladeSpeciesID, $SpeciesName) = $query->fetchrow_array()){
				...
				if(exists($TaxIDs_SFs->{$CladeSpeciesID})){
					...
					my $TaxLineage = CalculateLineage('',$CladeSpeciesID,$Viruses_id, $dbh);
					print OUTPUT $CladeSpeciesID."\t".$TaxLineage."\t 0 \n";
...
print OUTPUT "\n>Viral Species Labelled As 'no rank' in NCBI taxonomy\n\n#The following 'species' aren't labelled as such in the NCBI taxonomy tree (they hold 'no rank'). Rather than include the rest of the 'no rank' nodes in the tree, I thought that I'd include them here. There are $No_Of_Species_Unassigned of them, out of a total of $No_Of_Species_Total viral instances (species) in SUPERFAMILY.\n";
...
The output is a list of all viral instances in NCBI, their taxon_ids, thier full taxonomy and a list of the superfamilies contained within (if known). This list is broken down by viral classification
